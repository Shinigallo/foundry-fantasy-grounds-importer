 ...
 * nel modello dati del sistema dnd5e 3.3.1 (Foundry VTT v12).
 */
...
// Chiavi skill del sistema dnd5e
const SKILL_KEYS = {
  "acrobatics": "acr",
  "animal handling": "ani",
  "arcana": "arc",
...
// Abilità associata a ogni skill (dnd5e)
const SKILL_ABILITY = {
  acr: "dex", ani: "wis", arc: "int", ath: "str",
  dec: "cha", his: "int", ins: "wis", inv: "int",
  itm: "wis", nat: "int", per: "wis", prc: "cha",
  prf: "cha", rel: "int", sle: "dex", ste: "dex", sur: "wis"
};
...
/* ------------------------------------------------------------------ */
/*  Costruzione del payload dnd5e 3.3.1                                */
/* ------------------------------------------------------------------ */
...
export function buildActorData(pc) {
  ...
  // Saving throw: la prof di save vive su abilities.<k>.proficient
  ...
  for (const s of pc.skills) {
    ...
    if (key) skills[key] = { value: 1, ability: SKILL_ABILITY[key] };
  }
  ...
  // Classi: in dnd5e 3.x il livello/HD dell'actor è derivato dagli item di classe
  // (system.levels + system.hitDice, stringa tipo "d8").
  for (const c of parsed.classes) {
    items.push({
      name: c.name,
      type: "class",
      system: { levels: c.level, hitDice: hitDieFor(c.name) },
      flags: { [MODULE_ID]: { [FLAG_KEY]: true } }
    });
  }

  // Azioni -> armi. In dnd5e 3.x l'azione è inline sull'item (actionType/attack/damage),
  // senza activities. I totali FG includono già mod+prof, quindi attack.flat = true.
  for (const a of pc.actions) {
    ...
    const d = parsedAction.damage;
    const formula = `${d.number}d${d.denomination}${d.bonus !== null ? (d.bonus >= 0 ? "+" : "") + d.bonus : ""}`;
    items.push({
      name: a.name || "Action",
      type: "weapon",
      system: {
        ...
        damage: {
          parts: [[formula, d.types?.[0] || "bludgeoning"]],
          versatile: ""
        },
...
function hitDieFor(className) {
  ...
  if (/barbarian/.test(n)) return "d12";
  if (/(fighter|paladin|ranger)/.test(n)) return "d10";
  if (/(bard|cleric|druid|monk|warlock|artificer)/.test(n)) return "d8";
  return "d6";
}
 ...
 * Ritorna l'actor creato/aggiornato.
 */
async function importPc(pc, targetActor = null) {
  ...
  if (targetActor) {
    // Update: rimuoviamo gli item marcati dal modulo e aggiorniamo i campi
